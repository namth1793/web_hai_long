  ...
  if (notFound) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <Link to="/tin-tuc" className="btn-primary">{n.backToNews}</Link>
      </div>
    );
  }
    ...
    <>
      ...
      <div className="bg-gradient-to-r from-primary to-navy py-12">
        <div className="max-w-7xl mx-auto px-4">
          <Breadcrumb items={[{ label: n.header.breadcrumb, path: '/tin-tuc' }, { label: article.tieu_de.substring(0, 40) + '...' }]} />
        </div>
      </div>
              ...
              <div className="bg-white rounded-2xl overflow-hidden shadow-md">
                ...
                <div className="p-8 md:p-10">
                  <div className="flex flex-wrap items-center gap-4 mb-5 text-sm text-gray-500">
                    ...
                    <span className="flex items-center gap-1"><FiEye size={13} />{article.luot_xem} {n.views || 'views'}</span>
                  </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-gray-100 flex items-center gap-3 flex-wrap">
                    ...
                    {['Logistics', 'Import & Export', 'Hoàng Khang', article.danh_muc].map(tag => (
                  ...
                  <div className="mt-6">
                    <Link to="/tin-tuc" className="flex items-center gap-2 text-primary font-semibold hover:text-accent transition-colors text-sm">
                      <FiArrowLeft size={14} /> {n.backToNews}
                    </Link>
                  </div>
                </div>
              </div>
            ...
            <aside className="w-full lg:w-72 shrink-0 space-y-6">
              {article.related?.length > 0 && (
                <div className="bg-white rounded-2xl p-6 shadow-sm">
                  <h3 className="font-bold text-primary text-base mb-4 pb-3 border-b border-gray-100">{n.relatedTitle}</h3>
              ...
              <div className="bg-primary rounded-2xl p-6 text-white">
                <h3 className="font-bold text-base mb-3">{n.needConsult}</h3>
                <p className="text-blue-200 text-sm mb-4">{n.consultDesc}</p>
                ...
                <Link to="/lien-he" className="flex items-center justify-center gap-2 w-full border border-white/30 text-white py-2.5 rounded-lg font-semibold text-sm hover:bg-white/10 transition-colors">
                  {n.sendRequest} <FiArrowRight size={14} />
                </Link>
              </div>

              {/* Services Quick */}
              <div className="bg-white rounded-2xl p-6 shadow-sm">
                <h3 className="font-bold text-primary text-base mb-4 pb-3 border-b border-gray-100">{n.servicesTitle}</h3>
                <ul className="space-y-2">
                  {n.serviceLinks.map(s => (
                    <li key={s}>
